...
class Admin extends CI_Controller{
	...
	public function insert($tablename,$foldername,$prev_filename,$pagename)
	{
		...
		$columns = $this->Admin_model->table($tablename);
					for($i=0;$i<count($columns);$i++)
					{
						if(($columns[$i]!="id")&&($columns[$i]!="date_modified"))
						{
							if($columns[$i]=="status"){
								...
								$data[$columns[$i]]=$this->session->userdata('user_id');
							} else {
					...
					if($tablename == 'user'){
						$this->session->set_userdata('user',$this->input->post('fname'));
					...
					if($insert == true){
						redirect('View/'.$foldername.'/'.$pagename.'');
					}
					else{
						redirect('View/'.$foldername.'/'.$prev_filename.'');
					}
